...
@testset "state-belief consistency" begin
    ...
    for i in 1:5
        ...
        m = MetaMDP(max_step=5, prior=Beta(10rand(), 10rand()))
        heads = 0:m.max_step
        states = [Float64[] for h in heads]
        nostop_policy = RandomPolicy(m, 0)
        for i in 1:1000000
            roll = rollout(nostop_policy)
            push!(states[roll.b.heads + 1], roll.s)
        end

        # check that the distribution of states matches the belief
        foreach(heads, states) do h, ss
            b = Belief(m.max_step, h, m.max_step-h)
            empirical = fit(Beta, ss)
            analytic = posterior(m, b)
